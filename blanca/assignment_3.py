...
def run_monte_carlo(Ns: tuple[int, ...], seed: int = 42) -> list[float]:
    ...
    for n_samples in Ns:
        ...
        errors.append(abs(float(estimate[0]) - exact_value))
...
if __name__ == "__main__":
    ...
    cv_h1_errors, cv_h2_errors, cv_h3_errors = run_control_variates(Ns)

    is_beta_5_1_errors, is_beta_half_half_errors = run_importance_sampling(Ns)

    print("Absolute errors for Assignment 3\n")
    ...
    ax.loglog(Ns, mc_errors, marker="o", label="Standard MC")
    ax.loglog(Ns, cv_h1_errors, marker="o", label="Control variate h1")
    ax.loglog(Ns, cv_h2_errors, marker="o", label="Control variate h2")
    ax.loglog(Ns, cv_h3_errors, marker="o", label="Control variate h3")
    ax.loglog(Ns, is_beta_5_1_errors, marker="o", label="IS Beta(5, 1)")
    ax.loglog(
        Ns,
        is_beta_half_half_errors,
        marker="o",
    ...
    ax.set_title("Assignment 3: Monte Carlo variance reduction")
    ax.grid(True, which="both")
    ax.legend()

    fig.tight_layout()
    fig.savefig("assignment_3.png", bbox_inches="tight")
